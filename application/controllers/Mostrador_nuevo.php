...
class Mostrador_nuevo extends BaseController {

	public function nuevo()
	{
		$data=array();
			$data["titulo_descripcion"]="Nueva Venta";
		$this->vista("Mostrador_nuevo/index",$data);
	}
...
where detalle_id_sede=".$_COOKIE["id_sede"]." and tipo_documento.tipodoc_id<=6");
	...
	$data["tipo_documento_cliente"]=$this->db->query("select * from tipo_cliente where tipo_cliente_estado=1")->result_array();
		...
		$this->load->view("Mostrador_nuevo/nuevo",compact('data'));
	}
	...
	public function procesar_venta(){
		...
		$updatemov = array(
			'plan_contable_nrodoc' => $serie.'-'.$correlativo,
			'plan_contable_serie' => $serie,
			'plan_contable_correlativo' => $correlativo
		);
		$this->db->where('plan_contable_idreferencia',$id_movimiento);
		$this->db->update('plan_contable_comprobante', $updatemov);
	...
	public function detalleventa(){
		...
		if ($data["estadoventa"] == 1) {
			...
			$data["detalleventa"] = $this->Mantenimiento_m->consulta3("SELECT venta.venta_idventas,detalle_venta.descripcion,detalle_venta.cantidad,detalle_venta.observaciones,detalle_venta.estado_pedido,detalle_venta.detalle_estado_preparado,detalle_venta.cantidad,
				detalle_venta.cod_producto_venta,detalle_venta.precio,producto.producto_descripcion_
				FROM venta
