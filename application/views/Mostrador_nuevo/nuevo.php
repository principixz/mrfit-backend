...
<link rel="icon" type="image/png" sizes="16x16" href="<?php echo $data["empresa"]['empresa_icono'];?>">
...
<div id="tabs" class="tabs">
  ...
  <div id="contentenido" class="content">
    <section id="section-1">
      <div class="col-md-12">
        <div class="contents" id="" style="height: none;margin-top: 30px;">
    ...
    <section id="section-2">
      <div class="row">
              ...
              <div id="print" class="fixed-table-container">
                <div class="slimScrollDiv" style="position: relative; overflow: hidden; width: auto; height: 250px;">
                  <div id="list-table-div" style="overflow: hidden; width: auto; height: 255px;">
                    ...
                    <div id="cuerpo" style="margin-top: 5px;height: 190px !important;
        ...
        <div id="identrega" class="col-xs-3 col-md-3" style="padding-left:25px !important;margin-bottom: 10px;" >
            ...
            <div class="row">
              <div class="col-xs-6">
                <div class="form-group">
                  ...
                  <select id="cliente" class="form-control" name="cliente" onchange="cargar_direccion_nueva()" tabindex="-1" aria-hidden="true">
          ...
          <div class="row">
                  <div class="col-xs-6">
                    <div class="form-group">
                      ...
                      <input type="number" min="0" step="0.01" name="monto_delivery" onchange="deliverycosto()" onkeyup="deliverycosto()" id="monto_delivery" class="form-control" required="required">
                    </div>
                  </div> 
                     ...
                     <div class="col-md-6">
                      ...
                      <input type="hidden"  id="direccion_id1" name="direccion_id1" value="-">
                      <input placeholder="DIRECCIÓN" type="search" autocomplete="off" name="direccion_descripcio" id="direccion_descripcio" class="form-control"  value="">
...
<span onclick="consultar_externa()" id="basic-addon3" class="input-group-addon btn btn-success">Buscar</span>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input type="text" autocomplete="off" name="nombre_nuevo" id="nombre_nuevo" class="form-control" required="required">
                    </div>
                  </div>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      <label>Celular</label>
                      <input type="text" autocomplete="off" name="celular_nuevo" id="celular_nuevo" class="form-control" >
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="form-group">
                      <label>Direccion</label>
                      <input type="text" autocomplete="off" name="direccion_nuevo" id="direccion_nuevo" class="form-control" >
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="form-group">
...
<script src="<?php echo base_url() ?>public/modulos/mostrador.js"></script>
<script type="text/javascript" >
        ...
        {    
          ...
          if(obj["success"])
          {
            //alert(obj["result"]["nombre"]);
             $("#nombre_nuevo").val(obj["result"]["nombre"]+" "+obj["result"]["paterno"]+" "+obj["result"]["materno"]);
          }else{
...
fetch('https://ruc.selvafood.com/api/consultaruc/'+documento,{
 ...
 $("#basic-addon3").text("buscar");
        ...
        {    
          ...
          //console.log(datos);
          //var obj = JSON.parse(datos);
          $("#nombre_nuevo").val(obj["razon_social"]);
          if(obj["nombre_via"]!="-"){

             $("#direccion_nuevo").val(obj["tipo_via"]+" "+obj["nombre_via"]+" NRO "+obj["numero"]);
  ...
  function deliverycosto() {  
    if (isNaN(parseFloat($("#monto_delivery").val()) ) ) {
      dinero = parseFloat($("#totalparapagar").val()) ;
      $("#total_pagar_modal").text((parseFloat($("#totalparapagar").val())).toFixed(2));
      $("#dinero_entregado").val( dinero.toFixed(2) );
      generar_vuelto(); 
    }else{
      mont =  parseFloat($("#monto_delivery").val());
      dinero = parseFloat($("#totalparapagar").val()) + parseFloat(mont);
      $("#total_pagar_modal").text(dinero.toFixed(2));
      $("#dinero_entregado").val( dinero.toFixed(2) );
      generar_vuelto();
    }
      ...
      if (respuestastock == 3) { 
        $.post(base_url+"Mostrador/guardarpedido",JSON.stringify(datos_matriz),function(data){
          ...
          setTimeout(function(){
           if ($('#id_perfil') == 5) {
             window.close();
            // window.location.href = base_url+'Mostrador_nuevo'; 
           }else{        
            window.close();    
           //  window.location.href = base_url+'Mostrador_nuevo'; 
           }
...
function cobrar_venta(){
  ...
  if($("#monto_delivery").val()=="")
  {
    generar_notificacion("ERROR","AGREGAR PRECIO DE DELIVERY","error","");
    return 1;
  }
...
function generar_pago(){
 ...
 $("#totalparapagar").val(total_pagar);
// if ($("#cliente").val() == "") {
...
function cambio_direccion(event)
{
 // console.log(event);
...
function tipoentrega(id){
  ...
  switch(parseInt(id)) {
    ...
    html+=  '<input type="text" class="form-control" onkeypress="return solonumeros(event)" onkeyup="limpiar_informacion()"  maxlength="9"  required="true" name="ntelefono" id="ntelefono" autofocus="true" value="">';
        html+='<span onclick="mostrar_modal_nuevo_cliente()" style="cursor:pointer;" class="input-group-addon"><i class="fa fa-user"></i></span>';
       html+='</div>';

    html+='</div>';
    html+='<input type="hidden" id="idcliente" name="idcliente" value="">';
    html+='<input type="hidden" id="iddireccion" name="iddireccion" value="">';

        html+='<div class="form-group direccionclass">';        
    html+=  '<label class="control-label mb-10 text-left">DIRECCIÓN</label>';
    html+='<div class="input-group">';
    html+=  '<input type="text" class="form-control" onkeyup="cambio_direccion(event)"  required="true" name="direccion" id="direccion"  value="">';
    html+='<span onclick="crear_direccion_cliente()" style="cursor:pointer;" class="input-group-addon"><i class="fa fa-pencil"></i></span>';
  //  html+='<span onclick="llamarventas()" class="input-group-addon"><i class="fa fa-shopping-bag"></i></span>';
    ...
    html+=  '<label class="control-label mb-10 text-left">D.N.I</label>';
    html+=  '<input type="text" class="form-control" required="true" maxlength="8" name="dnidelivery" id="dnidelivery"  value="">';
    html+='</div>';
    //alert();
    ...
    if(data.estado == 2){
      ...
      for ( i = 0; i < data.ventas.length; i++) {
        ...
        for (var j=0; j < data["ventas"][i]["lista"].length; j++) {
         ...
         html+='<td  style=" border: 1px solid black;">'+data["ventas"][i]["lista"][j]["producto_descripcion_"]+'</td>';
...
function copiarventa(id){
  ...
  for(i=0; i < data["detalleventa"].length;i++ ){
   ...
   html+='<span class="sname" id="name_1526743222604">'+data["detalleventa"][i]["producto_descripcion_"]+'</span>';
   html+='</button>';
   html+='</td>';
   html+='</center>';
   html+='<center>';
        ...
        $.post(base_url+"Mostrador/detalleventa",{"id":idubicacion},function(data){
          if (data["estadoventa"] == 1) {
            ...
            for (var i = 0; i < data.detalleventa.length; i++) {
              ...
              html+='</td>';
              html+='</center>';
              html+='<center><td style="width:20%;text-align:center;" class="text-right"><span class="text-right " id="subtotaltraido'+data.detalleventa[i]["cod_producto_venta"]+'">'+subtotal+'</span></td></center>';
...
<?php if ($_COOKIE["usuario_perfil"] == 2) { ?>
  ...
  $(function(){
      ...
      $("#perfil").val("3").trigger('change');
         $("#ntelefono").autocomplete({
      ...
      source: function( request, response ) {
        ...
        $.ajax( {
          url: base_url+"pedido/buscar_cliente2",
          dataType: "json",
          data: {
            term: request.term,
      ...
      select: function( event, ui ) {
        ...
        //alert(ui.item.id);
         ...
         client=cli.split("/");
         $("#nombre").val(client[2]);
         $("#direccion").val(client[4]);
         $("#dnidelivery").val(client[1]);
         $("#iddireccion").val(client[3]);
        $("#idcliente").val(client[0]);
